...
// ======= (カテゴリ, ラベル, プリセット値は前回と同じ - 省略してimport) =======
// 前スクリプトと同じ定数を使用
const CATEGORIES_4 = [
...
const header = [
  ...
  '四麻選択肢1','四麻選択肢2','四麻選択肢3','四麻選択肢4','四麻選択肢5',
  '三麻選択肢1','三麻選択肢2','三麻選択肢3','三麻選択肢4','三麻選択肢5',
  '四麻:雀魂','四麻:Mリーグ','四麻:天鳳','三麻:関東三麻',
];
...
for (const key of orderedKeys) {
  ...
  const opts4 = opts4raw.filter(v=>v!=='カスタム'&&v!=='');
  const opts3raw = OPTIONS_3[key]||[];
  const opts3 = opts3raw.filter(v=>v!=='カスタム'&&v!=='');
  
  // 5列に展開
  const o4 = []; for (let i=0;i<MAX_OPTS;i++) o4.push(opts4[i]||'');
  const o3 = []; for (let i=0;i<MAX_OPTS;i++) o3.push(opts3[i]||'');
  ...
  rows.push([cat4,cat3,key,label,...o4,...o3,v_j,v_m,v_t,v_z].map(esc).join(','));
}
